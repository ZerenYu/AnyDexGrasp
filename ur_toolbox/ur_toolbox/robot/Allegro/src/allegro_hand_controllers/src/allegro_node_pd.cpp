  ...
  // Updating the torques in other conditions
  {
    ...
    if (desired_joint_state.position.size() == DOF_JOINTS) {
      ...
      for (int i = 0; i < DOF_JOINTS; i++) {
        ...
        // if (desired_torque[i] > max_tau_des and i != 14) desired_torque[i] = max_tau_des;
        // else if (desired_torque[i] > max_tau_des and i == 14 ) desired_torque[i] = max_tau_des + 0.1;
        // else if (desired_torque[i] < -max_tau_des and i != 14) desired_torque[i] = -max_tau_des;
        // else if (desired_torque[i] < -max_tau_des and i == 14) desired_torque[i] = -max_tau_des - 0.1;
        // #include <iostream>
        // using namespace std;
        // if (i == 14 and desired_torque[i] > 0) desired_torque[i] = desired_torque[i] + 1, cout >> desired_torque[i];
        // else if (i == 14 and desired_torque[i] < 0) desired_torque[i] = desired_torque[i] - 1, cout >> desired_torque[i];
        commanded_joint_states.effort[i] = desired_torque[i];
      }
    }
